<div class="container card my-4">
  ...
  <div>
    <a href="<?php echo base_url('/cargos'); ?>" class="btn btn-outline-primary regresar_Btn">Regresar</a>
  </div>
  ...
  <form method="POST" action="<?php echo base_url('/cargos/Restaurar'); ?>" class="form-check-inline">
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <span>
              <h3 class="text-center" id="CargoRestaurar"></h3>
            </span>
            <input type="text" id="idR" name="id" hidden>
            <input type="text" id="estado" name="estado" hidden>
          </div>
          <div class="modal-footer">
            <a href="<?php echo base_url('/cargos/eliminados') ?>"><button type="button close" class="btn btn-secondary" data-bs-dismiss="modal">Cerrar</button></a>
  ...
  function Restaurar(id) {
    ...
    $.ajax({
      ...
      success: function(rs) {
        $("#idR").val(rs[0]['id'])
        $("#estado").val('A')
        $("#CargoRestaurar").text(rs[0]['nombre']);
